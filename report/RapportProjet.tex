...
\begin{titlepage}
    \begin{center}
        % Logo of the school or company
        \includegraphics[width=0.4\textwidth]{figures/logo.png} \\[1.5cm]

        % Name of the school or company
        \textsc{\LARGE Grenoble INP - ENSE3}\\[1.5cm]

        % Report type
        \textsc{\Large Integration Project Report}\\[0.5cm]

        % Title of the report
        \HRule \\[0.4cm]
        { \huge \bfseries Modeling and Control of a Transport Network }\\[0.4cm]
        \HRule \\[1.5cm]
        ...
        \begin{minipage}{0.4\textwidth}
            \begin{flushleft} \large
                \emph{Authors:}\\
                Clément Milcent, Kevin Armel Wassa, Mathis Poteau
            \end{flushleft}
        \end{minipage}
        \begin{minipage}{0.4\textwidth}
...
Zone-based evacuation planning divides an area into zones, assigning each zone a specific evacuation path and departure schedule\cite{ref1}. This method enables precise management and effective communication of evacuation orders, and can be combined with contraflows to maximize network capacity. Additional constraints may be imposed on routes and departure times. This project focuses on modeling a transport network as a directed graph and formulating a Model Predictive Control (MPC) problem to optimize the overall system behavior over a finite time horizon.
...
\]
where $v_i$ denotes the velocity, $l_i$ the length of the cell, $C_i$ the maximum capacity, and $\gamma_i \in [0,1]$ is a control parameter used to regulate the flow. We can see its trend in figure~\ref{fig:demandfunction}.
...
\]
where $c_i$ is a capacity constant and $w_i$ is a saturation coefficient (figure~\ref{fig:supplyfunction}).
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/demandfunction.png}
        \caption{Demand function trend}
        \label{fig:demandfunction}
    \end{subfigure}
    \hfill
    % Sous-figure 2
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/supplyfunction.png}
        \caption{Semand function trend}
        \label{fig:supplyfunction}
    \end{subfigure}

    \caption{Demand and supply function trend}
    \label{fig:demandsupply}
\end{figure}
...
The simulator implements a discrete-time traffic flow model based on the Network Model. The road network is represented as a directed graph composed of tanks and road cells. Each cell is characterized by demand and supply functions, introducing nonlinearities and capacity constraints.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/schemabloc.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/line_graph/Simulator_Mpc_coupled/tank_L200.png}
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/line_graph/Simulator_Mpc_coupled/cells_L200.png}
        \caption{MPC cells states}
        \label{fig:MPC_cells_states}
    \end{subfigure}

    \caption{Line graphs of MPC states: tank and cells}
    \label{fig:MPC_states_linegraph_verif}
\end{figure}

The simulator matches accurately the MPC plots using MPC (figure~\ref{fig:MPC_states_linegraph_verif}). Especially the plots of the tanks and the cells $x_1,\ x_4,\ x_5$ and $x_6$. Regarding the other cells, the match is not perfect but the results are still really close. 
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/merge_graph/Simulator_Mpc_coupled/tanks_L200.png}
        \caption{MPC tank states}
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/merge_graph/Simulator_Mpc_coupled/cells_L200.png}
        \caption{MPC cells states}
        \label{fig:MPC_cells_states_many_cells}
    \end{subfigure}

    \caption{Y-shaped graphs of MPC states: tank and cells}
    \label{fig:MPC_states_many_cells}
\end{figure}

In the Y-shaped graph configuration, the results are slightly worse than for the line graph (figure~\ref{fig:MPC_states_many_cells}). Indeed, larger discrepancies can be observed between the MPC results and those of the simulator. However, the general trends are still respected, which indicates that the MPC is able to capture the main dynamics of the system despite the observed deviations.
...
Table~\ref{tab:tableComparisonOL_MPC} compares the performance of different control strategies in terms of evacuation time and cost. The results show that Model Predictive Control (MPC) with a prediction horizon of 1600 achieves an evacuation time of 744 s with a cost of 762,723, demonstrating consistent and efficient system management.

Open-loop strategies show more variability. The “Open-loop delayed evacuation” reaches a slightly faster time of 737 s but incurs a higher cost of 766,829, indicating less efficient resource usage. The “Open-loop simultaneous evacuation” matches the MPC evacuation time (744 s) with a slightly lower cost (761,588), yet its lack of feedback makes it less robust to disturbances or system variations. These results indicate that MPC is not superior in terms of time or cost under ideal conditions. 
\begin{table}[H]
    \centering
